...
pass `first-pass-5aa88d7f05b2` plus the versioned parser-format and analytical-scope-conflict
ledgers. It shows 1,938 document and 1,891 adjudication rows as
`single_coded_pending_human`, while 65 document exceptions, 61 adjudication exceptions, and all 486
exclusion-QA checks remain unstarted. The first pass may route a parser-warning source only to a
pending-human exclusion; separately recorded inclusions are explicitly source-coded and still
pending independent review. It is the operational starting point for continued review;
`full_corpus_review.html` remains the untouched-starter reference.
