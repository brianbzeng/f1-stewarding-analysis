...
|---|---:|---|
| `primary_candidate` | 250 | Race/Sprint source matched one and only one primary family |
| `secondary_candidate` | 66 | qualifying source matched only qualifying impeding |
| `manual_offence_review` | 388 | unclassified, conflicting, or overlapping family language |
| `manual_session_review` | 66 | no reliable session label and no higher-priority version/content disposition |
| `out_of_scope_suggestion` | 1,181 | recognized exclusion or out-of-scope session |
...
matches more than one family also stays manual rather than inheriting the first regex match.
