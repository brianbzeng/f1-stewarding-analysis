...
Status: machine-generated seed bundle and conservative first pass complete; independent source
review pending.
...
The first-pass scope, exceptions, hashes, and interpretation boundary are recorded in
[the machine-assisted first-pass release](full_corpus_first_pass.md).
...
The frozen patterns in `config/full_corpus_coding.yml` first classify text observed in the title,
Fact, and Infringement sections. If none matches, the Reason section may recover a primary-family
phrase hidden behind a generic legacy heading; it is never used to create a qualifying-impeding
candidate because negated phrases such as “did not impede” are common. The rules do not infer an
offence from penalty severity. The rule order reflects the protocol's primary families, the
qualifying-impeding secondary population, and common strict-liability or administrative exclusions.

When a parsed session is blank, schema v4 accepts only an explicit session phrase in the source
text; an ambiguous reference to a Race Director or a future race does not become a session label.
Known Practice and other out-of-scope sessions take precedence over lexical family conflicts. In a
qualifying session, only an explicit impeding match can enter the secondary candidate population;
other allegations remain outside that frozen secondary scope.
...
5. Review parser-warning documents from the PDF rather than treating missing headings as missing
   decisions. Legacy and bulk formats can contain valid adjudications.
6. Split documents that adjudicate multiple accused drivers into separate rows. Conversely, group
...
7. Deduplicate the final analytical version before creating `CodedAdjudication` records. A corrected
   successor becomes the source document; the predecessor remains provenance.
8. Independently review inclusion, incident grouping, offence family, outcome, and highlighted case
   studies. Record disagreement rather than silently overwriting the first code.
9. Publish an attrition table from 2,003 labels to final adjudications, with a count and reason for
...
Schema v4 also separates protected machine suggestions from reviewer-correctable
